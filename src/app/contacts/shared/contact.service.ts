...
export class ContactService {
  ...
  private insert(contact: Contact) {
    ...
    const sql = 'insert into contacts (name, email, tel) values (?, ?, ?)';
    const data = [contact.name, contact.email, contact.tel];
    
    return this.db.executeSQL(sql, data);
  }

  private update(contact: Contact) {
    const sql = 'update contacts set name = ?, email = ?, tel = ? where id = ?';
    const data = [contact.name, contact.email, contact.tel, contact.id];
